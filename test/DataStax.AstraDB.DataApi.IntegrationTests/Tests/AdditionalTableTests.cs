...
public class AdditionalTableTests
{
    ...
    [Fact]
    public async Task TestDictionaryTypes()
    {
        var tableName = "tableTestDictionaryTypes";
        try
        {
            var table = await fixture.Database.CreateTableAsync<DictionaryTypeTest>(tableName);

            List<DictionaryTypeTest> rows = new List<DictionaryTypeTest>();
            for (var i = 0; i < 5; i++)
            {
                rows.Add(new DictionaryTypeTest()
                {
                    Id = i,
                    ...
                    {
                        { DateTime.SpecifyKind(DateTime.Now.AddDays(i), DateTimeKind.Utc), $"Value {i}A" },
                        { DateTime.SpecifyKind(DateTime.Now.AddDays(i + 1), DateTimeKind.Utc), $"Value {i}B" }
                    },
                    ...
                    {
                        { 1.1m + i, $"DecimalValue {i}A" },
                        { 2.2m + i, $"DecimalValue {i}B" }
                    }
                });
            }
    ...
    [Fact]
    public async Task Test_CreateTable_IfNotExists()
    {
        var tableName = "tableCreateIfNotExists";
        try
        {
    ...
    [Fact]
    public async Task FindOne_Vector()
    {
        var tableName = "tableWithVector";
        try
        {
    ...
    [Fact]
    public async Task FindOne_Vector_UntypedFind()
    {
        var tableName = "tableWithVector_TestUntypedFind";
        try
        {
            var table = await fixture.Database.CreateTableAsync<SimpleObjectWithVector>(tableName);
            await table.CreateVectorIndexAsync("VectorEmbeddings_idx", (b) => b.VectorEmbeddings, Builders.TableIndex.Vector());
            ...
            var result = await tableUntyped.FindOneAsync(findOptions);
