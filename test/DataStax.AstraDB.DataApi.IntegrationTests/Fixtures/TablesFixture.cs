...
public class TablesFixture : BaseFixture, IAsyncLifetime
{
    ...
    private async Task CreateSearchTableVectorize()
    {
        try
        {
            var rows = new List<RowBookVectorize>() {
                new RowBookVectorize()
                {
                    Title = "Computed Wilderness",
                    Author = "Ryan Eau",
                    NumberOfPages = 22,
    ...
    private async Task CreateTestTablesNotTyped()
    {
        ...
        for (var i = 0; i < 50; i++)
        {
            ...
            if (IsAstra)
            {
                rowSingle["Vectorize"] = "String To Vectorize " + i;
            }
            rows.Add(rowSingle);
        }
